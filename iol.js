...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'cotizacion') {
      ...
      const res = await iolGet(`/api/v2/${mercado}/Titulos/${simbolo}/cotizacion`, token);
    ...
    if (action === 'multi') {
      ...
      await Promise.all(simbolos.map(async s => {
        try {
          const res = await iolGet(`/api/v2/${mercado}/Titulos/${s}/cotizacion`, token);
          results[s] = res.status === 200 ? JSON.parse(res.body) : { error: res.status };
    ...
    if (action === 'panel') {
      ...
      const token = await getToken(process.env.IOL_USER, process.env.IOL_PASS);
      const res = await iolGet(`/api/v2/${mercado}/Titulos/${panel}/cotizacion/paneles`, token);
      return { statusCode: res.status, headers, body: res.body };
    }
    ...
    if (action === 'portafolio') {
      const token = await getToken(process.env.IOL_USER, process.env.IOL_PASS);
      const res = await iolGet('/api/v2/micuenta/portafolio/arg', token);
      return { statusCode: res.status, headers, body: res.body };
    }
